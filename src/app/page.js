...
import historicalCars from '@/data/historical_cars.json';
import './font.css';

const PAIRED_ROWS = [
  { left: { idx: 1, name: "죽전마을 🌾" }, right: { idx: 34, name: "죽전마을 🌾" } },
  { left: { idx: 2, name: "서울슈퍼 🛒" }, right: { idx: 33, name: "서울슈퍼 🛒" } },
  { left: { idx: 3, name: "금성동주민센터 🏘️" }, right: { idx: 32, name: "금성동주민센터 🏘️" } },
  ...
  { left: { idx: 7, name: "고별대 🖐️" }, right: { idx: 28, name: "고별대 🖐️" } },
  { left: { idx: 8, name: "삼밭골 ⛰️" }, right: { idx: 27, name: "삼밭골 ⛰️" } },
  ...
  { left: { idx: 16, name: "온천장역 🚇" }, right: { idx: 18, name: "온천장역.SK허브스카이 🚇" }, loop: { idx: 17, name: "온천장 ♨️" }, isLoopBottom: true }
];
...
function BusIcon({ plateNumber }) {
  ...
  if (imgError) {
    return <div className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center bg-yellow-500 rounded-lg text-gray-900 font-bold text-xs md:text-sm shadow-inner">🚌</div>;
  }
  return <img src={imgSrc} alt={plateNumber} className="w-8 h-8 md:w-10 md:h-10 object-contain bg-gray-800 p-0.5 rounded-lg border border-gray-700 shadow-sm" onError={() => setImgError(true)} />;
}
...
export default function BusTracker() {
  ...
  return (
    <main className="w-full min-h-[101vh] bg-gray-950 text-gray-100 font-custom select-none">
      
      <div className="border-b border-gray-800 bg-gray-900/80 sticky top-0 backdrop-blur-md z-40 px-4 md:px-8 py-4 flex flex-col md:flex-row justify-between items-center gap-4 w-full shadow-md">
      ...
      <div className="max-w-5xl mx-auto px-2 md:px-4 py-8">
          ...
          <div className="w-full">
            <header className="mb-6 flex flex-col sm:flex-row justify-between items-center border-b border-gray-900 pb-5 gap-4">
              <div className="text-center sm:text-left">
                <h1 className="text-2xl md:text-3xl font-custom text-yellow-400 tracking-widest font-mono">203번 운행 현황</h1>
                <p className="text-[11px] md:text-xs text-gray-500 mt-2 tracking-wide break-keep">💡 정류장 이름을 클릭하면 주변에서 할 일과 유래를 볼 수 있어요!</p>
              </div>
            ...
            {loading ? (
              <div className="text-center text-gray-600 animate-pulse text-sm py-10">시스템 로딩 중...</div>
            ) : (
              <div className="w-full flex flex-col relative">
                
                <div className="grid grid-cols-[1fr_auto_15px_20px_15px_auto_1fr] md:grid-cols-[1fr_auto_40px_60px_40px_auto_1fr] text-center mb-4 text-[10px] md:text-xs font-bold text-gray-500 tracking-wider">
                  <div className="text-right pr-2 md:pr-4">하행 ↓</div>
                  <div></div><div></div><div></div><div></div><div></div>
                  <div className="text-left pl-2 md:pl-4">상행 ↑</div>
                </div>

                <div className="grid grid-cols-[1fr_auto_15px_20px_15px_auto_1fr] md:grid-cols-[1fr_auto_40px_60px_40px_auto_1fr] items-stretch relative">
                  {PAIRED_ROWS.map((row, rIdx) => {
                    ...
                    const loopBuses = row.loop ? buses.filter(bus => bus.bstopidx === row.loop.idx) : [];

                    return (
                      <div key={rIdx} className="contents">
                        
                        <div className="relative flex justify-end items-center h-full">
                          <div className={`absolute right-2 md:right-3 flex flex-col items-end gap-1.5 z-30 w-max ${row.isLoopBottom ? 'top-0' : 'top-1/2 -translate-y-1/2'}`}>
                              ...
                              {leftBuses.map((bus) => {
                                ...
                                return (
                                  <motion.div 
                                    ...
                                    className="flex items-center gap-1 md:gap-2 bg-gray-900/90 border border-blue-500/40 rounded-lg md:rounded-xl p-1 md:p-1.5 pl-2 md:pl-3 shadow-lg hover:border-yellow-400 cursor-pointer w-auto md:w-32 justify-between"
                                  >
                                    <div className="text-blue-400 font-black text-[10px] md:text-sm font-mono whitespace-nowrap">{plateNumber} ↓</div>
                        ...
                        <div onClick={() => handleStopClick(row.left)} className={`flex justify-end text-right pr-2 md:pr-4 text-[10px] md:text-xs font-semibold tracking-tight cursor-pointer hover:text-white transition-colors group ${leftBuses.length > 0 ? 'text-blue-400 font-bold' : 'text-gray-400'} ${row.isLoopBottom ? 'items-start pt-[10px] md:pt-[18px]' : 'items-center'}`}>
                          {row.left && (
                            <div className="flex flex-col md:flex-row items-end md:items-center justify-end w-full">
                              <span className="hidden md:inline text-[10px] text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity mr-1">🔍</span>
                              <span className="break-keep text-right">{row.left.name}</span>
                              <span className="text-[8px] md:text-[10px] text-gray-600 font-mono md:ml-1">#{row.left.idx}</span>
                            </div>
                          )}
                        </div>

                        {row.isLoopBottom ? (
                          <div className="col-start-3 col-end-6 relative flex justify-center min-h-[160px] md:min-h-[220px]">
                            <div className="absolute top-0 w-full h-[17px] md:h-[27px]">
                              <div className="absolute left-[7.5px] md:left-[20px] w-1 md:w-1.5 bg-blue-600 h-full -translate-x-1/2"></div>
                              <div className="absolute right-[7.5px] md:right-[20px] w-1 md:w-1.5 bg-blue-600 h-full translate-x-1/2"></div>
                            </div>

                            <div className="absolute top-[17px] md:top-[27px] left-[5.5px] md:left-[17px] right-[5.5px] md:right-[17px] h-[20px] md:h-[53px] border-l-[4px] border-r-[4px] border-b-[4px] md:border-l-[6px] md:border-r-[6px] md:border-b-[6px] border-blue-600 rounded-b-full"></div>

                            <div onClick={() => handleStopClick(row.left)} className={`absolute top-[17px] md:top-[27px] left-[7.5px] md:left-[20px] -translate-x-1/2 -translate-y-1/2 w-2.5 h-2.5 md:w-3.5 md:h-3.5 rounded-full z-10 cursor-pointer transition-all border-2 ${leftBuses.length > 0 ? 'bg-yellow-400 border-yellow-300 scale-125 shadow-[0_0_10px_rgba(234,179,8,0.6)]' : 'bg-gray-950 border-gray-600 hover:border-white'}`}></div>

                            <div onClick={() => handleStopClick(row.right)} className={`absolute top-[17px] md:top-[27px] right-[7.5px] md:right-[20px] translate-x-1/2 -translate-y-1/2 w-2.5 h-2.5 md:w-3.5 md:h-3.5 rounded-full z-10 cursor-pointer transition-all border-2 ${rightBuses.length > 0 ? 'bg-yellow-400 border-yellow-300 scale-125 shadow-[0_0_10px_rgba(234,179,8,0.6)]' : 'bg-gray-950 border-gray-600 hover:border-white'}`}></div>

                            <div onClick={() => handleStopClick(row.loop)} className={`absolute top-[37px] md:top-[80px] left-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 md:w-3.5 md:h-3.5 rounded-full z-10 cursor-pointer transition-all border-2 ${loopBuses.length > 0 ? 'bg-yellow-400 border-yellow-300 scale-125 shadow-[0_0_10px_rgba(234,179,8,0.6)]' : 'bg-gray-950 border-gray-600 hover:border-white'}`}></div>

                            <div onClick={() => handleStopClick(row.loop)} className={`absolute top-[49px] md:top-[96px] left-1/2 -translate-x-1/2 text-[10px] md:text-xs font-semibold tracking-tight cursor-pointer hover:text-white transition-colors z-20 flex flex-col items-center w-max ${loopBuses.length > 0 ? 'text-blue-400 font-bold drop-shadow-md scale-105' : 'text-gray-400'}`}>
                              {row.loop.name} <span className="text-[8px] md:text-[10px] opacity-60 font-mono">#{row.loop.idx}</span>
                            </div>

                            <div className="absolute top-[75px] md:top-[124px] left-1/2 -translate-x-1/2 flex flex-col gap-1.5 items-center z-30 w-max">
                              <AnimatePresence mode="popLayout">
                                {loopBuses.map((bus) => {
                                  const plateNumber = bus.carno.slice(-4);
                                  const cleanCarno = bus.carno.replace('부산', '').trim();
                                  ...
                                  return (
                                    <motion.div 
                                      ...
                                      key={`bus-${cleanCarno}`}
                                      onClick={() => setSelectedBus({ bus, details: details || null })} 
                                      className="flex items-center gap-2 md:gap-3 bg-gray-900/90 border border-blue-500/40 rounded-xl p-1.5 px-3 md:px-4 shadow-lg hover:border-yellow-400 cursor-pointer w-fit"
                                    >
                                      <div className="text-blue-400 font-black text-[10px] md:text-sm font-mono whitespace-nowrap">{plateNumber} →</div>
                                      <BusIcon plateNumber={plateNumber} />
                                    </motion.div>
                          ...
                          <>
                            <div className="relative w-full flex justify-center items-center min-h-[44px] md:min-h-[60px]">
                              <div className="absolute top-0 bottom-0 left-1/2 w-1 md:w-1.5 bg-blue-600 -translate-x-1/2"></div>
                              {row.left && <div onClick={() => handleStopClick(row.left)} className={`w-2.5 h-2.5 md:w-3.5 md:h-3.5 rounded-full z-10 cursor-pointer transition-all border-2 ${leftBuses.length > 0 ? 'bg-yellow-400 border-yellow-300 scale-125 shadow-[0_0_10px_rgba(234,179,8,0.6)]' : 'bg-gray-950 border-gray-600 hover:border-white'}`}></div>}
                            </div>
                            <div className="relative w-full min-h-[44px] md:min-h-[60px]"></div>
                            <div className="relative w-full flex justify-center items-center min-h-[44px] md:min-h-[60px]">
                              <div className="absolute top-0 bottom-0 left-1/2 w-1 md:w-1.5 bg-blue-600 -translate-x-1/2"></div>
                              {row.right && <div onClick={() => handleStopClick(row.right)} className={`w-2.5 h-2.5 md:w-3.5 md:h-3.5 rounded-full z-10 cursor-pointer transition-all border-2 ${rightBuses.length > 0 ? 'bg-yellow-400 border-yellow-300 scale-125 shadow-[0_0_10px_rgba(234,179,8,0.6)]' : 'bg-gray-950 border-gray-600 hover:border-white'}`}></div>}
                            </div>
                          </>
                        )}

                        <div onClick={() => handleStopClick(row.right)} className={`flex justify-start text-left pl-2 md:pl-4 text-[10px] md:text-xs font-semibold tracking-tight cursor-pointer hover:text-white transition-colors group ${rightBuses.length > 0 ? 'text-green-400 font-bold' : 'text-gray-400'} ${row.isLoopBottom ? 'items-start pt-[10px] md:pt-[18px]' : 'items-center'}`}>
                          {row.right && (
                            <div className="flex flex-col-reverse md:flex-row items-start md:items-center justify-start w-full">
                              <span className="text-[8px] md:text-[10px] text-gray-600 font-mono md:mr-1">#{row.right.idx}</span>
                              <span className="break-keep text-left">{row.right.name}</span>
                              <span className="hidden md:inline text-[10px] text-gray-600 opacity-0 group-hover:opacity-100 transition-opacity ml-1">🔍</span>
                            </div>
                          )}
                        </div>

                        <div className="relative flex justify-start items-center h-full">
                          <div className={`absolute left-2 md:left-3 flex flex-col items-start gap-1.5 z-30 w-max ${row.isLoopBottom ? 'top-0' : 'top-1/2 -translate-y-1/2'}`}>
                            <AnimatePresence mode="popLayout">
                              {rightBuses.map((bus) => {
                                const plateNumber = bus.carno.slice(-4);
                                const cleanCarno = bus.carno.replace('부산', '').trim();
                                ...
                                return (
                                  <motion.div 
                                    ...
                                    key={`bus-${cleanCarno}`}
                                    onClick={() => setSelectedBus({ bus, details: details || null })} 
                                    className="flex items-center gap-1 md:gap-2 bg-gray-900/90 border border-green-500/40 rounded-lg md:rounded-xl p-1 md:p-1.5 pr-2 md:pr-3 shadow-lg hover:border-yellow-400 cursor-pointer w-auto md:w-32 justify-between"
                                  >
                                    <BusIcon plateNumber={plateNumber} />
                                    <div className="text-left text-green-400 font-black text-[10px] md:text-sm font-mono whitespace-nowrap">↑ {plateNumber}</div>
                                  </motion.div>
                                );
                              })}
                            </AnimatePresence>
        ...
        {activeTab === 'records' && (
          <div className="w-full">
            <div className="mb-6">
              <h2 className="text-lg md:text-xl font-black text-white">역대 차량현황</h2>
              <p className="text-[11px] md:text-xs text-gray-500 mt-1 break-keep">203번 버스로 산성마을과 온천장 사이를 이어온 차량들의 상세 제원입니다.</p>
            </div>
            <div className="w-full overflow-x-auto bg-gray-900 border border-gray-800 rounded-2xl shadow-xl">
        ...
        {activeTab === 'behind' && (
          <div className="w-full space-y-6 pb-4">
            <div>
              <h2 className="text-lg md:text-xl font-black text-white">비하인드 스토리</h2>
              <p className="text-[11px] md:text-xs text-gray-500 mt-1 break-keep">공간 제약으로 노선도에 다 담지 못했던 깊은 이야기들...</p>
            </div>
                ...
                return (
                  <div key={story.id} className="bg-gray-900 border border-gray-800 rounded-2xl shadow-xl overflow-hidden transition-all duration-300">
                    <button 
                      ...
                      className="w-full text-left p-4 md:p-5 flex justify-between items-center hover:bg-gray-850/40 transition-colors group"
                    >
                      <h3 className={`text-sm md:text-base font-bold transition-colors duration-300 ${isOpen ? 'text-emerald-400' : 'text-gray-200 group-hover:text-emerald-400'}`}>
                        {story.title}
                      </h3>
                    ...
                    <div className={`grid transition-[grid-template-rows] duration-300 ease-in-out ${isOpen ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
                      <div className="overflow-hidden">
                        <div className="p-4 md:p-5 pt-0 text-xs md:text-sm text-gray-300 font-custom font-normal tracking-wide leading-relaxed md:leading-loose space-y-3 break-keep">
        ...
        {activeTab === 'credits' && (
          <div className="w-full bg-gray-900 border border-emerald-500/20 rounded-2xl p-4 md:p-6 shadow-xl space-y-6">
            <div>
              <h3 className="text-lg font-black text-white border-b border-gray-800 pb-2">크레딧</h3>
              <p className="text-gray-400 text-[11px] md:text-xs mt-3 leading-relaxed font-sans break-keep">
          ...
          <div className="bg-gray-900 border border-emerald-500/30 rounded-2xl max-w-md w-full p-6 shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3 text-xs leading-relaxed">
              <div className="bg-gray-950 p-4 rounded-xl border border-gray-800">
                ...
                <p className="text-gray-300 break-keep">{selectedStop.origin}</p>
              </div>
              <div className="bg-gray-950 p-4 rounded-xl border border-gray-800">
                <h4 className="text-yellow-400 font-bold mb-1.5">📍 추천하는 주변에서 할 일</h4>
                <p className="text-gray-300 break-keep">{selectedStop.spots}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4" onClick={() => setIsTimetableOpen(false)}>
          <div className="bg-gray-900 border border-emerald-500/30 rounded-2xl max-w-md w-full p-6 shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
            ...
            <header className="mb-4">
              ...
              <h3 className="text-xl md:text-2xl font-black text-white mt-1">🚌 203 운행 시간표 (평일)</h3>
            </header>
              ...
              <div className="bg-gray-950 border border-gray-800 rounded-xl p-3.5 space-y-2.5 text-gray-300">
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  <div className="bg-gray-900 p-2 rounded-lg border border-gray-800">
                    ...
                    <span className="text-gray-500 block text-[9px] md:text-[10px] mt-0.5">(00 / 15 / 30 / 45분)</span>
                  </div>
                  <div className="bg-gray-900 p-2 rounded-lg border border-gray-800">
                    <span className="text-gray-400 block mb-0.5">야간 (18:00 이후)</span>
                    <span className="font-mono text-white font-bold">20분 간격</span>
                    <span className="text-gray-500 block text-[9px] md:text-[10px] mt-0.5">(00 / 20 / 40분)</span>
                  </div>
                </div>
                <p className="text-[10px] text-gray-500 pt-1 break-keep">
                  * 평일 09:00 ~ 22:00 비수기 기간에는 야간과 동일하게 20분 간격으로 운행될 수 있습니다.
                </p>
              </div>

              <div className="text-[10px] md:text-[11px] text-gray-400 bg-emerald-950/20 p-3 rounded-xl border border-emerald-900/30 leading-relaxed break-keep">
